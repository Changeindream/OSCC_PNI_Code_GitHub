"""Legacy Swin Transformer needed for strict checkpoint compatibility.

The released Swin-MIL weights use the original down-sampling layout rather than
the layout in recent timm releases. This module is retained solely to reproduce
that architecture. It is derived from the Microsoft Swin Transformer and timm
implementations; see ``THIRD_PARTY_NOTICES.md``.
"""
...
from timm.layers import DropPath, Mlp, PatchEmbed, to_2tuple, trunc_normal_
from timm.models import build_model_with_cfg


def register_model(function):
    """Keep legacy decorators without mutating timm's global model registry."""
    return function

# 兼容不同版本的checkpoint_filter_fn导入
try:
...
def window_partition(x, window_size):
    ...
    Returns:
        windows (torch.Tensor): 划分后的窗口，形状为 (num_windows*B, window_size, window_size, C)
    """
    B, H, W, C = x.shape
...
def window_reverse(windows, window_size, H, W):
    ...
    Returns:
        x (torch.Tensor): 恢复后的特征图，形状为 (B, H, W, C)
    """
...
class WindowAttention(nn.Module):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.dim = dim
        self.window_size = window_size  # window_size 应为 int
...
class SwinTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=2,
        ...
        # 兼容不同版本获取 num_patches
        try:
            num_patches = self.patch_embed.num_patches
        except AttributeError:
            # 如果没有 num_patches 属性，手动计算
            img_size = to_2tuple(img_size)
            patch_size = to_2tuple(patch_size)
            num_patches = (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1])

        # 手动计算 patches_resolution
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
    ...
    def flops(self):
        flops = 0
        # 兼容不同版本的 patch_embed.flops()
        try:
            flops += self.patch_embed.flops()
        except (AttributeError, TypeError):
            # 如果 patch_embed 没有 flops 方法，手动计算
            flops += self.patch_embed.patch_size[0] * self.patch_embed.patch_size[1] * 3 * self.embed_dim * \
...
def _create_swin_transformer(variant, pretrained=True, default_cfg=None, **kwargs):
    if default_cfg is None:
        default_cfg = deepcopy(default_cfgs[variant])

    # 手动更新配置，替换 overlay_external_default_cfg 函数
...
def swin_small_patch4_window7_224(pretrained=False, **kwargs):
    """Swin-S @ 224x224, pretrained ImageNet, fine tune"""
    model_kwargs = dict(
...
def swin_tiny_patch4_window7_224(pretrained=False, **kwargs):
    """Swin-T @ 224x224, pretrained ImageNet, fine tune"""
    model_kwargs = dict(
        patch_size=4, window_size=7, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), **kwargs)
